<template>
<div ref="wrap" id="wrap" class="vue-slide-bar-component vue-slide-bar-horizontal" @click="wrapClick">
    <div ref="elem" class="vue-slide-bar" :style="{height: `${lineHeight}px`}" id="slider">
        <template>
            <div ref="tooltip" class="vue-slide-bar-always vue-slide-bar-tooltip-container" :style="{'width': `${iconWidth}px`}" @mousedown="moveStart" @touchstart="moveStart">
                <span class="vue-slide-bar-tooltip-top vue-slide-bar-tooltip-wrap" v-if="showTooltip">
            <slot name="tooltip">
              <div class="vue-slide-bar-tooltip" ></div>
            </slot>
          </span>
            </div>
        </template>
...
export default {
    ...
    data() {
        return {
            ...
            interval: 1000, //how much interval we needed
            lazy: false,
            realTime: false,
            sliderMax: this.max,
            updateTime: this.max,
    ...
    computed: {
        ...
        maximum() {
            return this.data ? (this.data.length - 1) : this.sliderMax
        },
        multiple() {
    ...
    methods: {
        ...
        scrollToTime(time) {
            this.setCurrentValue(time);
        },
        scrollRight() {
            this.setCurrentValue(this.currentValue + 1000);
        },
        scrollLeft() {
            this.setCurrentValue(this.currentValue - 1000);
        },
        ...
        wrapClick(e) {
            ...
            EventBus.$emit(TimeLinePanelEvent.SCROLLTOTIME, this.currentValue);
        },
        moveStart(e, index) {
            this.flag = true
        ...
        moveEnd(e) {
            ...
            this.flag = false
            this.setPosition()
            console.log("this. move end " + this.currentValue);
            EventBus.$emit(TimeLinePanelEvent.SCROLLTOTIME, this.currentValue);
        },
        ...
        limitValue(val) {
            ...
            const inRange = (v) => {
                if (v < this.min) {
                    ...
                    return this.min
                } else if (v > this.sliderMax) {
                    this.printError(`[VueSlideBar warn]: The value of the slider is ${val}, the maximum value is ${this.sliderMax}, the value of this slider can not be greater than the maximum value`)
